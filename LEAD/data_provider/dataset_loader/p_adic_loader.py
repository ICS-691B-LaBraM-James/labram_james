...
def get_id_list_p_adic(args, data_list: np.ndarray, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for P-ADIC data
    ...
    de_list = list(data_list[np.where(data_list[:, 0] == 3)][:, 1])  # Depression IDs
    sc_list = list(data_list[np.where(data_list[:, 0] == 4)][:, 1])  # Schizophrenia IDs
